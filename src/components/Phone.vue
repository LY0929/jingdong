<template>
  <div class="content">
    <ul>
      <router-link v-for="(phone,index) in allPhone" tag="li" :to="{name:'detailp',params:{bid:phone.id}}" :key="index">
        <div class="img">{{phone.id}}</div>
...
<script>
  import {getPhone} from "../api/getData";
...
<style scoped>
  ...
  .content .on{width:60px;height:12px;line-height:12px;margin-top:15px;font-size:10px;color: red;border:1px solid red}
